...
function checkTodoFixmeComments(pr: PRData): SlopCheck {
  let todoCount = 0
...
function checkCopiedStackOverflow(pr: PRData): SlopCheck {
  ...
  for (const file of pr.files) {
    ...
    for (const line of addedLines) {
      for (const pattern of suspiciousPatterns) {
        if (pattern.test(line)) { matchCount++; break }
      }
    }
  }
...
function checkGeneratedCodeMarkers(pr: PRData): SlopCheck {
...
function checkDescriptionMatchesChanges(pr: PRData): SlopCheck {
  const body = (pr.body || '').toLowerCase()
  if (!body || body.length < 20) return { name: 'description-matches-changes', description: 'Description references actual changed files/code', passed: true, severity: 'medium' }
